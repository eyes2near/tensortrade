...
def _create_wallet_source(wallet: 'Wallet', include_worth: bool = True) -> 'List[Stream[float]]':
    ...
    with NameSpace(exchange_name + ":/" + symbol):
        ...
        if include_worth:
            price = None
            try:
                price = Stream.select(wallet.exchange.streams(), lambda node: node.name.endswith(symbol+"_close"))
            except:
...
class TensorTradeObserver(Observer):
    ...
    **kwargs : keyword arguments
    ...
    def __init__(self,
                 ...
                 **kwargs) -> None:
        ottercontext = True
        if not hasattr(Stream.stream_thread_local, 'reset_member'):
            ottercontext=False
            Stream.stream_thread_local.reset_member = set()
        ...
        self.warmup()
        if not ottercontext:
            del(Stream.stream_thread_local.reset_member)
