...
class SimpleOrders(TensorTradeActionScheme):
    ...
    def get_orders(self,
        ...
        #print("actions.py -> ", "\n\tnet_worth->",portfolio.net_worth,"\n\tinstrument->",instrument,"\n\tinstrument precision->",instrument.precision,"\n\tbalance->", balance,"\n\tsize->", size,"\n\tquantity->", quantity,"\n\tmin_order_pct->", self.min_order_pct,"\n\tmin_order_abs->", self.min_order_abs,"\n\tep->", ep)
        
        if size < 10 ** -instrument.precision \
                or size*float(ep.price) < self.min_order_pct * portfolio.net_worth \
                or size*float(ep.price) < self.min_order_abs:
            return []
