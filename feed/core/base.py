...
class Stream(Generic[T], Named, Observable):
    ...
    stream_thread_local = threading.local()
...
class IterableStream(Stream[T]):
    ...
    def reset(self, random_start=0):
        ...
        #print('i am ', self.name,', id = ',id(self), ', reset called random_start = ', random_start)
        if random_start != 0:
        ...
        if self.is_gen:
            ...
            if hasattr(self.iterable, 'iloc'):
                self.generator = iter(self.iterable.iloc[self._random_start:])
            else:
